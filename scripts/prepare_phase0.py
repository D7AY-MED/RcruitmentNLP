...
def main():
    print("--- Starting Phase 0 Preparation (Local Schema Mode) ---")
    today = datetime.date.today().strftime("%Y%m%d")
    
    # 1. Create backups directory
    backup_dir = os.path.join("backups")
    env_backup_dir = os.path.join(backup_dir, f"env_{today}")
    os.makedirs(env_backup_dir, exist_ok=True)
    print(f"Created backup directories: {backup_dir} and {env_backup_dir}")
    
    # 2. Copy active .env files
    env_files = [
        (".env", ".env"),
        ("frontend/.env", "frontend_env"),
        ("backend/.env", "backend_env")
    ]
    for src, name in env_files:
        if os.path.exists(src):
            dest = os.path.join(env_backup_dir, name)
            shutil.copy(src, dest)
            print(f"Copied {src} to {dest}")
        else:
            print(f"Skipped {src} (does not exist)")

    # 3. Reconstruct Database SQL Schema from local resources
    print("Reconstructing database schema from local migrations and documents...")
    sql_output = []
    sql_output.append("-- PooLink Supabase Schema Backup (Local Reconstruction)")
    sql_output.append(f"-- Generated on: {datetime.datetime.now().isoformat()}")
    sql_output.append("-- This schema represents public tables, indexes, constraints, and RLS policies.\n")
    
    # 3.1 candidate_profiles (from frontend/supabase/migrations/00001_create_candidate_profiles.sql)
    sql_output.append("""-- Table Structure: public.candidate_profiles
...
""")
    ...
    # 3.2 hr_profiles (reconstructed from metadata usage)
    sql_output.append("""-- Table Structure: public.hr_profiles
...
""")
    ...
    # 3.3 admin_profiles (from frontend/supabase/migrations/00002_create_admin_profiles.sql)
    sql_output.append("""-- Table Structure: public.admin_profiles
...
""")
    ...
    # 3.4 job_pools (from frontend/supabase/migrations/00003_create_job_pools.sql)
    sql_output.append("""-- Table Structure: public.job_pools
...
""")
    ...
    # 3.5 interview_sessions (from docu/AI_INTERVIEW.md & backend/migrations/002_add_pool_id.sql)
    sql_output.append("""-- Table Structure: public.interview_sessions
...
""")
    ...
    # 3.6 recruiters (from backend/app/models/recruiter.py)
    sql_output.append("""-- Table Structure: public.recruiters (FastAPI Local SQLite/PG Table)
...
""")
    ...
    schema_path = os.path.join(backup_dir, f"supabase_schema_{today}.sql")
    with open(schema_path, "w", encoding="utf-8") as sf:
        sf.write("\n".join(sql_output))
    print(f"SQL Schema successfully reconstructed and exported to {schema_path}")
