...
export default function CandidateOffersPage() {
  ...
  return (
    <div className="bg-slate-50 dark:bg-bg min-h-screen relative overflow-hidden flex flex-col font-sans">
      ...
      {/* ---------- HEADER ---------- */}
      <header className="w-full z-50 sticky top-0 bg-white/80 dark:bg-card/80 backdrop-blur-md border-b border-border-brand py-4">
      ...
      <section className="relative py-12 px-4 sm:px-6 lg:px-8 text-center z-10 max-w-4xl mx-auto mt-6">
        <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-900 dark:text-ink tracking-tight mb-4">
          Découvrez nos <span className="text-brand">opportunités d'emploi</span>
        </h1>
        <p className="text-lg text-slate-650 dark:text-ink max-w-2xl mx-auto leading-relaxed font-normal">
          Explorez des postes à forte valeur ajoutée, postulez facilement et passez un entretien vidéo intelligent propulsé par notre IA.
        </p>
      </section>

      {/* ---------- FILTERS & SEARCH ---------- */}
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 pb-20 relative z-10">
        <div className="bg-white dark:bg-card p-4 sm:p-6 rounded-3xl border border-border-brand shadow-xl mb-8 flex flex-col md:flex-row gap-4 items-center justify-between">
          {/* Search bar */}
          <div className="relative w-full md:flex-1">
            <Search className="absolute left-4 top-3.5 h-5 w-5 text-gray-400 dark:text-muted" />
            <input
              type="text"
              placeholder="Rechercher un poste, une entreprise ou une ville..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-surface border border-border-brand rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand transition-all"
            />
          </div>
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                  (type === 'All' && contractFilter === 'All') || (contractFilter.toLowerCase() === type.toLowerCase())
                    ...
                    : 'bg-slate-50 dark:bg-surface text-gray-650 dark:text-ink hover:bg-slate-105 dark:hover:bg-surface-2 border border-border-brand'
                }`}
              >
        ...
        {/* ---------- OFFERS LIST ---------- */}
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-405 dark:text-muted">
          ...
          <div className="space-y-4">
            {filteredOffers.map((offer) => (
              <div
                ...
                className="bg-white dark:bg-card border border-border-brand rounded-3xl p-6 sm:p-8 shadow-sm hover:shadow-xl hover:border-brand/40 hover:-translate-y-1 transform transition-all duration-300 cursor-pointer flex flex-col md:flex-row justify-between items-start md:items-center gap-6"
              >
                {/* Offer Details */}
                <div className="space-y-3 flex-1">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-500 dark:text-muted mt-0.5">
                        {offer.company_name || 'Entreprise Confidentielle'}
                      </p>
                    </div>
                  </div>

                  <p className="text-sm text-slate-650 dark:text-ink line-clamp-2 leading-relaxed">
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-2">
                    {offer.location && (
                      <span className="inline-flex items-center gap-1 px-3 py-1 rounded-lg bg-slate-50 dark:bg-surface border border-border-brand text-xs font-semibold text-slate-550 dark:text-muted">
          ...
          <div className="bg-white dark:bg-card border border-border-brand rounded-3xl p-12 text-center shadow-md max-w-md mx-auto relative overflow-hidden">
            <div className="w-16 h-16 rounded-2xl bg-slate-50 dark:bg-surface border border-border-brand flex items-center justify-center mx-auto mb-4">
              <Inbox className="w-8 h-8 text-slate-400 dark:text-muted" />
            </div>
            <h3 className="text-lg font-bold text-ink mb-1">Aucune offre disponible</h3>
            <p className="text-xs text-slate-500 dark:text-muted leading-relaxed mb-6">
